...
const ShowCategory = () => {
  ...
  useEffect(() => {
    ...
    axios
      .get(
        `https://assi11-mim-dots-projects.vercel.app/articles?sort=${category}`
      )
      .then((res) => {
        setData(res.data);
        setLoading(false);
      ...
      .catch((err) => {
        console.error(
          "Fail https://assi11-mim-dots-projects.vercel.app/:",
          err
        );
        setLoading(false);
      });
      ...
      {["Science", "Arts", "Technology"].map((category, index) => (
        ...
        >
          ...
          <p className="mt-3 text-sm text-center font-medium opacity-80">
            ✨ Explore the world of <span className="italic">{category}</span>{" "}
            articles.
          </p>
